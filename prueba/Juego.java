...
public class Juego {
    ...
    public boolean jugar(List<Palabra> todasLasPalabras, String categoria, boolean modoDios) {
        ...
        secreta = secreta.toUpperCase(); // Convertir a mayúsculas
        
        // Crear el arreglo de guiones
        ...
        if (modoDios == true) {
            // Revelar primera letra automáticamente
    ...
    public Palabra seleccionarPalabraAleatoria(List<Palabra> lista, String categoria) {
        ...
        // Obtener un número aleatorio entre 0 y el tamaño de la lista
    ...
    public boolean validarLetra(char letra, String palabra, char[] estado) {
        ...
        for (int i = 0; i < palabra.length(); i++) {
            ...
            if (letraEnPalabra == letra) {
                // Reemplazar el guion por la letra
                if (estado[i] == '_') {
                    estado[i] = letra;
                }
                encontrada = true;
    ...
    public boolean estaCompleta(char[] estado) {
        for (int i = 0; i < estado.length; i++) {
            char c = estado[i];
            if (c == '_') {
                return false; // Si hay un guion, no ha ganado
            }
        }
        return true; // Si terminó el ciclo y no hay guiones, ya ganó
    }
